...
```js 1:3 subtitle="ES5 Function Component"
function Component1() {
  return <h1>Hello, World!</h1>;
}
...
```js 5:7 subtitle="ES6 Fat-Arrow Function Component"
function Component1() {
  return <h1>Hello, World!</h1>;
}
...
```js 9 subtitle="ES6 Fat-Arrow Function Component with implicit return"
function Component1() {
  return <h1>Hello, World!</h1>;
}
...
```js 11:15 subtitle="ES6 Class Component"
function Component1() {
  return <h1>Hello, World!</h1>;
}
...
```js 2 subtitle="Use curly-braces to drop into JavaScript expression"
function MyComponent1(props) {
  return <h1>Hello, {props.name}!</h1>;
}
...
```js 5:8 subtitle="You can also assign other JSX to a variable to inject"
function MyComponent1(props) {
  return <h1>Hello, {props.name}!</h1>;
}
...
```js 10:12 subtitle="Or you an destructure the value as it comes inside the component"
function MyComponent1(props) {
  return <h1>Hello, {props.name}!</h1>;
}
...
```js 4:10 subtitle="You can show one JSX or another based on a truthy expression"
function MyComponent(props) {
  return (
    <h1>
      {props.name ? (
...
```js 4:10 subtitle="You can hide or show content based on a truthy expression"
function MyComponent(props) {
  return (
    <h1>
      {props.name ? (
...
```js 4:7 subtitle="You can use the && operator to also hide/show if the expression is boolean"
function MyComponent(props) {
  return (
    <h1>
      {!!props.name && (
...
```js 1,4:6 subtitle="You could manually add props with matching names"
function MyComponent(props) {
  return (
    <SubComponent
      name={props.name}
...
```js 1:2 subtitle="Or you could spread the properties from the object"
function MyComponent(props) {
  return <SubComponent {...props} />;
}
```

```js 4:5 subtitle="Spreading is like Object.assign in that it merges from right to left"
function MyComponent(props) {
  return (
    <SubComponent
      {...props}
...
```js 1:4 subtitle="Sometimes you don't want to spread everything"
function MyComponent(props) {
...
```js 2:6 subtitle="Another technique is using lodash's omit"
function MyComponent(props) {
...
```js 4 subtitle="Style prop takes an object, not a string"
function MyComponent(props) {
  return (
    <h1
      style={{ backgroundColor: "black", color: "white" }}
...
```js 2,6 subtitle="There's an implicit children prop"
function MyComponent(props) {
...
```js title="React.useState function"
...
```js 4 subtitle="Call React.useState and provide a default value"
import React, { useState } = "react";
...
```js 6,7 subtitle="Returned array has value & update function"
import React, { useState } = "react";
...
```js 4 subtitle="You can use array destructuring instead"
import React, { useState } = "react";
...
```js title="React.useEffect function"
...
```js 4 subtitle="Call the React.useEffect function"
import React, { useEffect } from "react";
...
```js 5:10 subtitle="Use React.useEffect and provide a callback"
import React, { useEffect } from "react";
...
```js 11:12 subtitle="Return an optional clean-up function"
import React, { useEffect } from "react";
...
```js 5:12 subtitle="By default callback & clean-up executed before each effect"
import React, { useEffect } from "react";
...
```js 13 subtitle="Can optionally provide 2nd argument to adjust frequency"
import React, { useEffect } from "react";
...
```js title="React.useReducer function"
...
```js 8:11 subtitle="Call the React.useEffect passing a reducer function and initialState"
const initialState = { count: 0 };
...
```js 1 subtitle="Initial state is an object with default values"
const initialState = { count: 0 };
...
```js 4:11 subtitle="Reducer accepts existing state and incoming action"
const initialState = { count: 0 };
...
```js 4:6,22 subtitle="dispatch triggers reducer to get engaged"
const initialState = { count: 0 };
